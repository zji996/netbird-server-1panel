...
msg() {
  ...
  case "$LANGUAGE:$key" in
    ...
    en:progress_apply_1panel) echo "Writing 1Panel OpenResty root.conf" ;;
    en:progress_firewall_short) echo "Opening firewall ports" ;;
    en:progress_firewall) echo "Checking firewall and allowing TCP %s, UDP %s when supported" ;;
    en:progress_start_services_short) echo "Starting containers" ;;
    en:progress_start_services) echo "Starting containers with Docker Compose. First run may pull images and take a few minutes..." ;;
    en:progress_stop_services) echo "Stopping containers with Docker Compose..." ;;
    en:progress_restart_services) echo "Recreating containers with Docker Compose..." ;;
    en:progress_setup_admin) echo "Preparing NetBird admin account" ;;
    en:progress_status_checks) echo "Checking service status and endpoints" ;;
    en:admin_setup_waiting) echo "Waiting for NetBird setup API (%s/%s): %s" ;;
    ...
    zh:progress_apply_1panel) echo "写入 1Panel OpenResty root.conf" ;;
    zh:progress_firewall_short) echo "放行防火墙端口" ;;
    zh:progress_firewall) echo "检查防火墙，并在支持时放行 TCP %s、UDP %s" ;;
    zh:progress_start_services_short) echo "启动容器" ;;
    zh:progress_start_services) echo "正在通过 Docker Compose 启动容器。首次运行可能需要拉取镜像，会花几分钟..." ;;
    zh:progress_stop_services) echo "正在通过 Docker Compose 停止容器..." ;;
    zh:progress_restart_services) echo "正在通过 Docker Compose 重建容器..." ;;
    zh:progress_setup_admin) echo "准备 NetBird 管理员账号" ;;
    zh:progress_status_checks) echo "检查服务状态和端点" ;;
    zh:admin_setup_waiting) echo "等待 NetBird 初始化接口（%s/%s）：%s" ;;
