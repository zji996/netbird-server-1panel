...
server:
  listenAddress: ":80"
  exposedAddress: "${PUBLIC_SCHEME}://${DOMAIN}:${PUBLIC_PORT}"
  stunPorts:
    - ${STUN_PORT}
  metricsPort: 9090
  ...
  auth:
    issuer: "${PUBLIC_SCHEME}://${DOMAIN}/oauth2"
    signKeyRefreshEnabled: true
    dashboardRedirectURIs:
      - "${PUBLIC_SCHEME}://${DOMAIN}/nb-auth"
      - "${PUBLIC_SCHEME}://${DOMAIN}/nb-silent-auth"
    cliRedirectURIs:
...
render_dashboard_env() {
  cat <<EOF
# Generated by netbird-server-tui.sh
NETBIRD_MGMT_API_ENDPOINT=${PUBLIC_SCHEME}://${DOMAIN}
NETBIRD_MGMT_GRPC_API_ENDPOINT=${PUBLIC_SCHEME}://${DOMAIN}
AUTH_AUDIENCE=netbird-dashboard
AUTH_CLIENT_ID=netbird-dashboard
AUTH_CLIENT_SECRET=
AUTH_AUTHORITY=${PUBLIC_SCHEME}://${DOMAIN}/oauth2
USE_AUTH0=false
